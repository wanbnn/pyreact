...
def create_post_version(base_version: str, run_id: int, run_attempt: int) -> str:
    ...
    post_number = run_id * 100 + run_attempt
...
def update_ci_version(root: Path, run_id: int, run_attempt: int) -> str:
    """Update build metadata and the runtime version, returning the new version."""
    pyproject = root / "pyproject.toml"
    package_init = root / "pyreact" / "__init__.py"
    version = create_post_version(
        _read_base_version(pyproject), run_id=run_id, run_attempt=run_attempt
    )
    _replace_version(pyproject, PROJECT_VERSION, version)
    _replace_version(package_init, PACKAGE_VERSION, version)
    return version
